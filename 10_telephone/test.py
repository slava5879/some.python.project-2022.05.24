...
def test_bad_seed_str():
    ...
    bad = random_string()
    rv, out = getstatusoutput(f'py {prg} -s {bad} {fox}')
    assert rv > 0
...
def test_bad_mutation_str():
    ...
    bad = random_string()
    rv, out = getstatusoutput(f'py {prg} -m {bad} {fox}')
    assert rv > 0
...
def test_bad_mutation():
    ...
    for val in ['-1.0', '10.0']:
        rv, out = getstatusoutput(f'py {prg} -m {val} {fox}')
        assert rv > 0
...
def test_for_echo():
    ...
    rv, out = getstatusoutput(f'py {prg} -m 0 "{txt}"')
    assert rv == 0
...
def test_now_cmd_s1():
    ...
    txt = open(now).read().rstrip()
    rv, out = getstatusoutput(f'py {prg} -s 1 "{txt}"')
    assert rv == 0
    expected = """
...
def test_now_cmd_s2_m4():
    ...
    txt = open(now).read().rstrip()
    rv, out = getstatusoutput(f'py {prg} -s 2 -m .4 "{txt}"')
    assert rv == 0
    expected = """
...
def test_fox_file_s1():
    ...
    rv, out = getstatusoutput(f'py {prg} --seed 1 {fox}')
    assert rv == 0
...
def test_fox_file_s2_m6():
    ...
    rv, out = getstatusoutput(f'py {prg} --seed 2 --mutations .6 {fox}')
    assert rv == 0
    txt = open(fox).read().rstrip()
