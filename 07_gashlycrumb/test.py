...
def test_bad_file():
    ...
    rv, out = getstatusoutput(f'py {prg} {letter} -f {bad}')
    assert rv != 0
...
def test_a():
    ...
    rv, out = getstatusoutput(f'py {prg} a')
    assert rv == 0
...
def test_b_c():
    ...
    rv, out = getstatusoutput(f'py {prg} b c')
    assert rv == 0
...
def test_y():
    ...
    rv, out = getstatusoutput(f'py {prg} Y')
    assert rv == 0
...
def test_o_alternate():
    ...
    rv, out = getstatusoutput(f'py {prg} o P q -f alternate.txt')
    assert rv == 0
...
def test_bad_letter():
    ...
    rv, out = getstatusoutput(f'py {prg} 5 CH')
    assert rv == 0
