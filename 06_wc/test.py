...
def test_bad_file():
    ...
    bad = random_string()
    rv, out = getstatusoutput(f'py {prg} {bad}')
    assert rv != 0
...
def test_empty():
    ...
    rv, out = getstatusoutput(f'py {prg} {empty}')
    assert rv == 0
...
def test_one():
    ...
    rv, out = getstatusoutput(f'py {prg} {one_line}')
    assert rv == 0
...
def test_two():
    ...
    rv, out = getstatusoutput(f'py {prg} {two_lines}')
    assert rv == 0
...
def test_fox():
    ...
    rv, out = getstatusoutput(f'py {prg} {fox}')
    assert rv == 0
...
def test_more():
    ...
    rv, out = getstatusoutput(f'py {prg} {fox} {sonnet}')
...
def test_stdin():
    ...
    rv, out = getstatusoutput(f'cmd /c "py {prg} < {fox}"')
    assert rv == 0
