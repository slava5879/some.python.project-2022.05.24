...
def test_bad_vowel():
    ...
    rv, out = getstatusoutput(f'py {prg} -v x foo')
    assert rv != 0
    assert re.match("usage", out, re.IGNORECASE)
...
def test_command_line_with_vowel():
    ...
    out = getoutput(f'py {prg} -v i foo')
...
def test_command_line_with_vowel_preserve_case():
    ...
    out = getoutput(f'py {prg} "APPLES AND BANANAS" --vowel i')
...
def test_file_with_vowel():
    ...
    out = getoutput(f'py {prg} --vowel o {fox}')
